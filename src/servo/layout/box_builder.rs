...
impl LayoutTreeBuilder {
    ...
    */
    ...
    fn make_image_box(&self,
        ...
        let element = node.as_image_element();
        if element.image.is_some() {
            let holder = ImageHolder::new(copy *element.image.get_ref(), layout_ctx.image_cache);
    ...
    fn make_text_box(&self,
        ...
        let string = &node.as_text().text;

        // FIXME: Don't copy text. I guess it should be atomically reference counted?
        @UnscannedTextBox(RenderBoxData(node, ctx, self.next_box_id()), copy *string)
    }

    fn decide_box_type(&self, node: Node, display: CSSDisplay) -> RenderBoxType {
        if node.is_text() {
            RenderBox_Text
        } else if node.is_image_element() && node.as_image_element().image.is_some() {
            RenderBox_Image
