...
pub fn parse_html(url: Url,
    ...
    unsafe {
        // Build the root node.
        let root = ~HTMLHtmlElement { parent: Element::new(HTMLHtmlElementTypeId) };
        let root = unsafe { Node::as_abstract_node(root) };
        debug!("created new node");
        let parser = hubbub::Parser("UTF-8", false);
        debug!("created parser");
        parser.set_document_node(root);
        ...
        let append_hook: @fn(AbstractNode, AbstractNode) = |parent_node, child_node| {
            if parent_node.is_style_element() && child_node.is_text() {
                ...
                let data = &child_node.as_text().text;
                let provenance = InlineProvenance(result::unwrap(url), copy *data);
        ...
        parser.set_tree_handler(@hubbub::TreeHandler {
            create_comment: |data: ~str| {
                ...
                unsafe {
                    Node::as_abstract_node(~Comment::new(data))
                }
            },
            ...
            create_element: |tag: ~hubbub::Tag| {
                ...
                match element.node.type_id {
                    ...
                    ~ElementNodeTypeId(HTMLImageElementTypeId) => {
                        let image_element = node.as_image_element();
                        do image_element.element.get_attr(~"src").iter |img_url_str| {
                ...
                unsafe {
                    Node::as_abstract_node(element)
                }
            },
            create_text: |data: ~str| {
                debug!("create text");
                unsafe {
                    Node::as_abstract_node(~Text::new(data))
                }
            },
            ref_node: |_| {},
            unref_node: |_| {},
