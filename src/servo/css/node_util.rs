...
impl NodeUtil for AbstractNode {
    ...
    fn get_css_select_results(self) -> &self/CompleteSelectResults {
        ...
        match node.layout_data {
            None => fail!(~"style() called on a node without aux data!"),
            Some(layout_data) => {
    ...
    fn set_css_select_results(self, decl: CompleteSelectResults) {
        let node = self.as_node();
        match node.layout_data {
            None => fail!(~"set_css_select_results() called on a node without aux data!"),
            Some(layout_data) => layout_data.style = Some(decl)
        }
    }
}
